...
```{r}
cran3 <- cran3 %>% mutate(size_gb = size / 2**10)
```
...
```{r}
by_package %>% summarise(valor_medio = mean(size))
```
...
pack_sum <- by_package %>% summarise(.count = n(), .unique = n_distinct(ip_id),
                                     .countries =
                           n_distinct(country), .avg_bytes = mean(size),
...
top_count <- pack_sum %>% filter(.count > quantile(.count, probs = 0.99))
top_count
...
top_count_sorted <- top_count %>% arrange(-.count)
top_count_sorted
```
...
top_unique_sorted <- pack_sum %>% filter(.unique >
                                           quantile(.unique,
                                                    probs = 0.99)) %>% arrange(desc(.unique))
top_unique_sorted
...
```{r}
print(cran %>% select(c(ip_id,country,package,size)))
...
print(cran %>% filter(size_mb >= 0.5) %>% arrange(-size_mb))
